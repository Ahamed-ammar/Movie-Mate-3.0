...
const PopularReviewCard = memo(({ review, getPosterUrl, movieId, formatNumber }) => {
  ...
  const starRating = useMemo(() => {
    if (!review.ratingStars) return null;
    const fullStars = Math.floor(review.ratingStars);
    const hasHalfStar = review.ratingStars % 1 >= 0.5;
          ...
          {starRating && (
            <div className="flex items-center gap-1 ml-2">
              <div className="flex text-green-400 text-sm">
                ...
                {starRating.hasHalfStar && <span>½</span>}
